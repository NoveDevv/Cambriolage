        ...
        RMenu.Add("cambriolage2", "script_main", RageUI.CreateMenu("Cambriolage","Sortir de cette maison !!"))
    ...
    Citizen.CreateThread(function()
        ...
        RageUI.IsVisible(RMenu:Get("cambriolage2","script_main"),true,true,true,function() 
            ...
            RageUI.ButtonWithStyle("Sortir", "Sors de la maison !", {RightLabel = ""}, true,function(h,a,s)
                if s then
                    RageUI.CloseAll()
                    SetEntityCoords(GetPlayerPed(-1),1289.5472, -1710.999, 55.4746, false, false, false, true)
                    ESX.ShowAdvancedNotification("Cambriolage ", "~r~Cambriolage !","Merci tu nous a pris beaucoup d'argent je te recontacterais plus tard" , "CHAR_LESTER_DEATHWISH", 24,5)
                end
            end)
